...
app.put('/snacks/:snackId', (req, res, next) => {
  const updatedSnack = {};
  updatedSnack.id = vendingMachine[req.snackIndex].id;
  updatedSnack.name = req.body.name;
  updatedSnack.proce = req.body.price;
  vendingMachine[req.snackIndex] = updatedSnack;
  res.send(vendingMachine[req.snackIndex]);
});
